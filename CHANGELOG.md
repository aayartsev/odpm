...
- **Base image profile override (4.7.0-beta)** — optional `ODPM_BASE_IMAGE_PROFILE` (`full` / `medium` / `ci`) in process env or layered `.env` overrides the scenario default Dockerfile profile without changing other `ScenarioPolicy` knobs; invalid values fall back with a warning. ADR-007 amendment. Tests: `test_dockerfile_profiles`. Docs: `env-dotenv.md`, `adr-007-base-image-profiles.md`, `developer.md`, `ci.md`, `server.md`, `generated-files.md`.
- **Scenario-aware odoo_conf frozen keys (ADR-022)** — global frozen (`addons_path`, `data_dir`, `admin_passwd`, `http_port`) in every scenario; `db_*` allowed only in effective `ci` (external DB via ordinary `scenarios.ci.odoo_conf.options`, optional `${@secret:}`); derived `ci_manifest_db_override` skips false `template.odoo_conf` regen and `db_host_mismatch` drift. Tests: `test_manifest_odoo_conf`, `test_manifest_scenario_overrides`, `test_odoo_conf_ci_db_override`. Docs: ADR-022, `odoo-conf.md`, `odpm-json.md`, `ci.md`.
- **Secrets providers + Infisical (ADR-021)** — `SecretsProvider` protocol (`file` + Infisical Universal Auth via stdlib REST, no extra pip deps); one fetch per process (`ensure_secrets_source` + session); prepare step `secrets.fetch` before `secrets.materialize`; `secrets.provider` in manifest v2 (overlay replaces the object); CLI `--secrets-provider` and env `ODPM_SECRETS_PROVIDER` / `INFISICAL_*`; CI bake always `ensure`s on the same session. `--plan` + `${@secret:}` may hit the API on setup. Tests: `test_secrets_providers_resolve`, `test_secrets_provider_file`, `test_secrets_provider_infisical`, `test_infisical_client`, `test_secrets_fetch_prepare`. Docs: ADR-021, `secrets.md`, `odpm-json.md`, `env-dotenv.md`, `cli.md`.
- **CI prepare-only policy (4.7+)** — `SystemCheckPolicy` skips Docker daemon / compose CLI probe for `ci` + `kaniko` + `direct` + `--skip-start`/`--build-image`; structural compose validate stays on; bare `odpm` in `ci` rejected unless allowlisted; layered dotenv wired into image-builder resolve; ADR-017. Tests: `test_system_check_policy`, `test_odpm_pipeline`, `test_ci_image_build_backends`. Docs: `ci.md`, `env-dotenv.md`, `tools-by-scenario.md`.
- **Scenario-driven env wizard (4.7+)** — interactive `.env` asks `ODPM_SCENARIO` first; CI branch prompts builder/Kaniko mode (default `direct`)/registry/push without ports; ADR-018. Tests: `test_user_env_ci_wizard`, `test_user_env_debugger`. Docs: `env-dotenv.md`.
- **Kaniko base image build (4.7+)** — base image uses `ImageBuildBackend`; with `kaniko`, base always pushes to `ODPM_BASE_IMAGE_REGISTRY` (fail-fast if unset); final push stays independent; ADR-019. Tests: `test_project_env_modules`, `test_ci_image_build_backends`. Docs: `ci.md`, ADR-016 cross-link.
- **Compose `privileged` / `pid` on sidecars (4.7+)** — whitelist `privileged` (boolean) and `pid` (string, e.g. `host` / `service:<name>`) on manifest `services` / `service_patches` (schema + passthrough + `${VAR}` / `${@service:}` / `${@secret:}` expand in `pid`); structural compose checks. Tests: `test_manifest_v2_reader`, `test_env_substitution`, `test_compose_validate`, `test_compose_fragments`. Docs: `odpm-json.md`, `plugins.md`, ADR-009.
- **CI image build backends (4.7+)** — pluggable `docker` (default) and `kaniko` for `odpm --build-image`; CLI `--image-builder` / `--image-push` and env `ODPM_CI_IMAGE_BUILDER` / `ODPM_CI_IMAGE_PUSH` / Kaniko executor settings; ADR-016. Tests: `test_ci_image_build_backends`. Docs: `ci.md`, `cli.md`, `env-dotenv.md`.
- **Compose `hostname` / `healthcheck` on sidecars (4.7+)** — whitelist `hostname` and Compose-shaped `healthcheck` on manifest `services` / `service_patches` (schema + passthrough + `${VAR}` / `${@service:}` / `${@secret:}` expand in strings); structural compose checks for hostname/healthcheck types. Tests: `test_manifest_v2_reader`, `test_env_substitution`, `test_compose_validate`, `test_compose_fragments`. Docs: `odpm-json.md`, ADR-009.
- **Manifest `odoo_conf` extra INI sections (4.7+)** — `odoo_conf` may declare module sections (`redis_server`, `s3_server`, …) beside `options`; schema + deep merge + `${@secret:}` / `${@service:}` expand. Reserved keys still apply only to `options`. Tests: `test_manifest_odoo_conf`, `test_odoo_conf_builder`. Docs: `odpm-json.md`, `odoo-conf.md`.
- **`${@secret:<key>}` in manifest (4.7+)** — expand values from `.odpm/secrets.json` into `services` / `service_patches` / hooks / `odoo_conf` (dotted keys); requires existing secrets file or `--secrets-file` (import before Config bootstrap); placeholders rejected; intentional duplication into generated compose YAML for sidecars. Tests: `test_env_substitution`, `test_secret_refs`, `test_odpm_pipeline`. Docs: `odpm-json.md`, `secrets.md`, `plugins.md`.
- **`${@service:<name>}` in compose fields (4.7+)** — logical compose service refs (`db`, `odoo`, sidecars) expand to physical names from `ODPM_COMPOSE_PREFIX` / `POSTGRES_SERVICE_NAME` in whitelist `services` / `service_patches` strings (`environment`, `command`, …) and hook argv; ADR-012 amendment. Docs: `docs/reference/odpm-json.md`, `docs/reference/env-dotenv.md`. Tests: `test_env_substitution`, `test_compose_service_prefix`, `test_compose_service_names`.
...
- **Golden-path refresh accepts baseline DB drift in CI** — `refresh_golden_path_project.sh` / remedi ate pass `--accept-database-drift` for `postgres_major`, `odpm_scenario`, and `data_dir_empty_changed` (no TTY); still not `data_path` / `app_role_missing`. If Postgres stays down after a major bump, wipe volume + remedi ate; fail-fast when `odoo_version` is not `19.x`. Tests: `test_golden_path`. Docs: `docs/contributing/ci.md`.
- **Server scenario uses git clone for platform** — same as `developer`: prepare `git.materialize` clones the platform repo; removed legacy nightly zip download (`PlatformSourcesService`), free-space/Yandex Disk helpers, and `get_platform_sources`. Pin via `odoo_build_date` / `--odoo-build-date`. Docs: `docs/scenarios/server.md`, `docs/operations/non-interactive.md`.
...
- **Compose stack network from `.env` (4.7)** — optional `ODPM_COMPOSE_NETWORK` / `ODPM_COMPOSE_NETWORK_EXTERNAL` for one named network (managed bridge or external proxy) attached to the full stack; implicit default when unset; prefix rewrite for managed networks; manifest validate warns on `networks: ["stack"]` mismatch. ADR-014. Tests: `test_compose_network`, `test_compose_validate`, extended `test_manifest_scenario_overrides`, `test_user_env_bootstrap`, `test_env_substitution`.
