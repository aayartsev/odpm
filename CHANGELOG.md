...
- **Golden core venv (4.7+)** — reuse core `.venv` (Odoo requirements, no project extras) keyed by `venv_lock_hash` for fresh scenarios; `ODPM_GOLDEN_VENV` / `ODPM_GOLDEN_VENV_ROOT`; clone via `clonevirtualenv` or freeze+hardlink with EXDEV→copy fallback; compose mount `/cache/odpm/venvs`; layered dotenv wired into host mounts and `ODPM_GOLDEN_VENV` injected into compose `odoo` environment; `.incomplete` marker against populate races; gevent `--no-build-isolation` on freeze path. Tests: `test_golden_venv`, `test_virtualenv_checker`, `test_bake_venv`, `test_compose_service_prefix`. Docs: `docs/reference/golden-venv.md`.
- **Shared wheel cache (4.7+)** — `ODPM_WHEEL_CACHE_ROOT` (default `~/.odpm/cache`) shares pip/uv download caches across projects; compose mounts `/cache/odpm/...` for developer/server; layered dotenv + process env merge for mounts and host CI bake; live stderr tee for pip/uv; never overrides explicit `PIP_CACHE_DIR` / `UV_CACHE_DIR`. Tests: `test_wheel_cache`, `test_bake_venv`, `test_volume_mapper`, `test_user_env_bootstrap`, `test_ci_image_build_backends`. Docs: `docs/reference/wheel-cache.md`, `env-dotenv.md`.
