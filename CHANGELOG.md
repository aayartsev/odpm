...
- **`dev_project/config/manifests/`** — `UserSettingsReader` and `OdpmJsonReader` extract manifest path setup and JSON load from `ConfigLoader`; `ConfigLoader` delegates via thin wrappers. Default factories (`create_default_user_setting_json_content`, `rewrite_odpm_json`) stay on loader until C-2. `OdpmJsonReaderTests`, `UserSettingsReaderTests`, and loader delegation tests in `test_host_config`.
