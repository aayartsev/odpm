...
- **Golden-path refresh accepts baseline DB drift in CI** — `refresh_golden_path_project.sh` / remedi ate pass `--accept-database-drift` for `postgres_major`, `odpm_scenario`, and `data_dir_empty_changed` (no TTY); still not `data_path` / `app_role_missing`. Tests: `test_golden_path`. Docs: `docs/contributing/ci.md`.
