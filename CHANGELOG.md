...
- **`dev_project/project_env/environment.py` (E-5)** — `CreateProjectEnvironment` slimmed to prepare wiring only (`templates`, `compose_generator`, `links`, `mapped_folders`, system-checker attachment); runtime delegates removed. `OdpmPipeline.configure_vscode` uses `VscodeConfigurator` directly. `RuntimeProjectServicesProtocol` deprecated; env no longer implements it. `test_protocols_and_imports` updated.
