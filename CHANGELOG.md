...
- **Golden-path CI gate ≤9 min** — `release-packages.yml` / `ci-docker.yml` job `timeout-minutes: 9`; pre-release refresh uses `ODPM_GOLDEN_PATH_AUTO_REMEDIATE=1` (remedi ate only when schema incompatible: `base` 19.x + `web` + `res_lang.short_time_format`); HTTP wait `ODPM_GOLDEN_PATH_TIMEOUT=60`. Install step: `sudo -n` fail-fast + `DEBIAN_FRONTEND=noninteractive` + `timeout 60` for `dpkg`; narrow NOPASSWD template `scripts/ci/github-actions-runner-sudoers.example`. Docs: `docs/contributing/ci.md`, ADR-006.
