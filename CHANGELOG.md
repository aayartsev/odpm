...
- **`dev_project/project_env/services/platform_sources.py`** — `PlatformSourcesService` extracts `download_odoo_nightly_build` from `CreateProjectEnvironment`; `SystemChecker.check_file_system` wires nightly download for server scenario. Developer platform git clone uses `GitRepoCoordinator.get_platform_sources`. `PlatformSourcesServiceTests` in `test_project_env_links`.
