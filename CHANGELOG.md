...
- **`dev_project/config/bootstrap_context.py`** — `ConfigBootstrapContext` replaces `ConfigLoader` as composition root for manifests, artifacts, defaults, and transforms; bootstrap and layout call context services directly. **`dev_project/config/loader.py` removed**. `ConfigBootstrapContextWiringTests`, `BindDevelopingLinkTests`; `config._loader` mocks removed from tests.
