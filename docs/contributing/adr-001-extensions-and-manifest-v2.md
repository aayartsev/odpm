...
|------|------------------|---------|---------|
| Product / CLI | `RELEASE_VERSION`, `ODPM_VERSION`, `odpm --version`, pip wheel, deb/rpm | `4.5.0` | Installed odpm version (single user-facing value) |
| Manifest schema | `manifest_schema` in JSON | `1`, `2` | Shape and semantics of `odpm.json` |
| Min manager (v2 only) | `requires_odpm` | `4.5.0` | Semver: installed manager must be ≥ this |
...
- User-facing version is unified: `RELEASE_VERSION` = `ODPM_VERSION` = pip/deb/`odpm --version` (currently `4.5.0`).
