...
| HTTP 500, в postgres: `translate IS TRUE must be type boolean` | БД создана старой версией Odoo. Пересоздать БД под текущий Odoo 19 (backup → drop DB / новый volume Postgres → `odpm -d test_db --odoo-bin -i base,web --stop-after-init`) или прогнать миграцию Odoo вне golden-path CI. На pre-release CI `refresh_golden_path_project.sh` делает это автоматически (`ODPM_GOLDEN_PATH_INIT_MODULES`, по умолчанию `base,web`), если `ODPM_GOLDEN_PATH_AUTO_REMEDIATE` не `0`. Каталог `data/postgresql/...` часто **root-owned** — wipe в CI через `docker run … alpine rm`, не `find` от пользователя runner. Если `translate=character varying` после init — проверьте `odoo_version` в `odpm.json` (нужен 19.0). |
