...
| HTTP 500, в postgres: `translate IS TRUE must be type boolean` | БД создана старой версией Odoo (несовместимая схема под Odoo 19). Пересоздать БД (backup → drop DB / wipe volume Postgres → `odpm -d test_db --odoo-bin -i base,web --stop-after-init`). Preflight в CI проверяет **`base` 19.x + `web` installed** в `ir_module_module`, не тип SQL-колонки `ir_model_fields.translate` (на Odoo 19 она остаётся `character varying`). На pre-release `refresh_golden_path_project.sh` делает auto-remediate (`ODPM_GOLDEN_PATH_INIT_MODULES=base,web`). Wipe volume — через `docker run … alpine`, не `find` от пользователя runner. |
