...
|-----------|--------------|---------------|
| `RELEASE_VERSION` | Каждый релиз / pre-release на `4.4-dev` | `4.4.2` (stable); следующий pre-release — по плану |
| `LATEST_STABLE_RELEASE` | **Только** при выходе **stable** тега (без `-beta`/`-rc`) | `4.4.2` |
