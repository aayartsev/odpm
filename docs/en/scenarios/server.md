...
| **Base image** | Default profile **medium** (wkhtmltopdf + build deps, no browser/Xvfb/IDE). Override with `ODPM_BASE_IMAGE_PROFILE` = `full` / `medium` / `ci` — see [ADR-007](../../contributing/adr-007-base-image-profiles.md). |
