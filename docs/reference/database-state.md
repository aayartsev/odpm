...
При подключённом TTY odpm задаёт вопросы (принять новый data path, создать роль, показать инструкции wipe и т.д.). Ответ **«принять и продолжить»** обновляет `.odpm/database/last_run.json` до текущей конфигурации. Без TTY — ошибка с перечислением KIND; используйте **`--accept-database-drift=KIND`** (флаг можно повторять; baseline тоже обновляется).

### Смена major PostgreSQL

odpm **не мигрирует** data dir между major-версиями. В интерактивном prompt вариант **(c)** выводит пошаговую инструкцию: остановить контейнеры, удалить каталог данных, затем:
