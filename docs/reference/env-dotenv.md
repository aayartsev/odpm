...
| `ODPM_BASE_IMAGE_PROFILE` | Override профиля base Dockerfile: `full`, `medium` или `ci` (слабее process env; невалидное значение → дефолт сценария). Не меняет остальную политику сценария — см. [ADR-007](https://github.com/aayartsev/odpm/blob/4.7.0-dev/docs/contributing/adr-007-base-image-profiles.md) | из `ODPM_SCENARIO` |
...
Допустимо: `full`, `medium`, `ci`. Process env сильнее layered `.env`. См. [ADR-007](https://github.com/aayartsev/odpm/blob/4.7.0-dev/docs/contributing/adr-007-base-image-profiles.md).
