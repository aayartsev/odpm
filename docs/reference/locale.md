...
Поле **`database.language`** в [odpm.json](odpm-json.md) (или устаревшее **`db_lang`** в `user_settings.json`) задаёт язык **базы данных** Odoo при создании (`ru_RU`, `en_US`), а не язык сообщений самой утилиты odpm. При наличии обоих источников действует манифест.
