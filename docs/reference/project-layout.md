...
**Каталог odpm-проекта** (например `odoo_demo_project-19/`) — это **рабочая оболочка окружения** на вашем компьютере. Исходники модулей могут физически лежать в другом месте (в `ODOO_PROJECTS_DIR` или по пути `file://`).
