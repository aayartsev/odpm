...
| **Предупреждения Python в логах** | В `docker-compose` задаётся `PYTHONWARNINGS=ignore::DeprecationWarning:docutils`, чтобы при старте Odoo не пугать новичка длинным traceback от устаревшего API в пакете **docutils** (это не ошибка вашего кода). В сценариях `server` и `ci` фильтр **не** включается — там предупреждения остаются в журнале и могут сигнализировать о несовместимости docutils с версией Python в образе. |
