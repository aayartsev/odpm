...
| **Предупреждения Python в логах** | Переменная `PYTHONWARNINGS` **не** задаётся: при старте Odoo в журнале могут появляться `DeprecationWarning` от **docutils** (часто с traceback). Это шум от зависимости в виртуальном окружении, а не признак поломки модулей проекта; при необходимости обновите docutils в requirements или зафиксируйте совместимую версию. В сценарии `developer` odpm такие сообщения скрывает намеренно. |
