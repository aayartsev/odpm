...
# parser.add_argument('--init', help='init project with default params it takes over 10 -15 minutes')
...
project_config_path = "%s/%s"%(project_dir, project_config_file)
...
def check_dir_content(full_dir_path):
    ...
    return dirs_list, files_list, dir_is_module, dir_is_git_repo
...
if dev_project_url:
    ...
    # run_command('mkdir -p %s/%s'%(projects_dir,project_name))
...
if not modules_to_update:
    modules_to_update =[project_dir_from_url]
...
if project_dir_from_url:
    if os.path.exists(os.path.join(odoo_projects_dir,project_dir_from_url)):
        addons_string = addons_string + os.path.join(odoo_projects_dir,project_dir_from_url) + ","
