...
- **HTTP smoke (T2, mandatory on `4.5-dev` PR):** `HttpSmokeIntegrationTests` when `ODPM_RUN_HTTP_SMOKE=1` — Mailpit `compose up` + HTTP 200 (`./scripts/run_http_smoke_test.sh`). See [ADR-006](../../docs/contributing/adr-006-integration-gate-policy.md).
