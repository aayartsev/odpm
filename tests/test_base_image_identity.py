...
class BaseImageIdentityTests(unittest.TestCase):
    ...
    def test_write_and_read_round_trip(self):
        with tempfile.TemporaryDirectory() as project_dir:
            identity = {"user": "alice", "uid": "1001", "gid": "1001"}
    ...
    def test_identity_mismatch_when_scenario_changes(self):
        with tempfile.TemporaryDirectory() as project_dir:
            dev_config = self._config(project_dir, constants.DEVELOPER_SCENARIO)
            write_base_image_identity(
                project_dir, expected_base_image_identity(dev_config)
            )
            ci_config = self._config(project_dir, constants.CI_SCENARIO)
            if dev_config.policy.runtime_unix_user() == ci_config.policy.runtime_unix_user():
                self.skipTest("host user is odoo; dev and ci identity coincide")
            self.assertFalse(base_image_identity_matches(ci_config))
