...
# Test paths
TEST_BASE_DIR = Path("/tmp/odoo_fork_test_projects")
...
def run_odpm_script(work_dir: Path, version: str) -> tuple[bool, float]:
    ...
    cmd = [
        ...
        str(odpm_script),
        "--init",
        "git@github.com:example/acme_extra_module.git",
        "--odoo-version",
        "17.0",
        "--python-version",
        "3.10",
        ...
        # "dev",
        "--odoo-git-link",
        "'git@github.com:example/acme-odoo-17.git dev'",
        "--platform-name",
        "acme",
        "--requirements-txt",
        """cryptography==42.0.8,dadata==21.10.1,decorator==5.2.1,docxcompose==2.1.0,docxtpl==0.20.2,google-auth==2.31.0,pdfminer.six==20260107,pyOpenSSL==24.1.0,pyasn1==0.4.1,pymorphy2==0.9.1,pymorphy3==2.0.6,python-docx==1.2.0,python-jose==3.4.0,pytils==0.4.4,redis==5.0.7,requests==2.31.0,pycairo==1.29.0,urllib3==2.0.7,yookassa==3.3.0,setuptools==80""",
        "-d",
        "test_db",
        "--odoo-bin",
        "--stop-after-init",
...
def main():
    ...
    if CLEAN_DOCKER_IMAGES:
        delete_images()
    try:
        for version in BRANCHES_TO_TEST:
            _logger.info("-" * 60)
            ...
            port_offset = BRANCHES_TO_TEST.index(version) * 10
