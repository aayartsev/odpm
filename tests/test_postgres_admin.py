...
import dev_project.database.ensure_role as ensure_role_mod
import dev_project.database.postgres_admin as postgres_admin
...
class RunPsqlAsAdminTests(unittest.TestCase):
    @patch.object(postgres_admin, "resolve_psql_admin_role", return_value="odoo")
    @patch.object(postgres_admin, "_run_psql")
    def test_uses_resolved_admin_role(self, mock_run, _mock_resolve):
        ...
        mock_run.assert_called_once_with(
            config, "odoo", "postgres", "-tAc", "SELECT 1", exec_user="postgres"
        )

    @patch.object(postgres_admin, "resolve_psql_admin_role", return_value=None)
...
class BootstrapAppRoleSingleUserTests(unittest.TestCase):
    ...
    @patch.object(postgres_admin, "_wait_for_postgres_ready")
    @patch.object(postgres_admin, "compose_up_service_detached")
    @patch.object(postgres_admin, "compose_run")
    @patch.object(postgres_admin, "compose_stop_service")
...
class EnsureAppRoleAdminIntegrationTests(unittest.TestCase):
    @patch.object(ensure_role_mod, "run_psql_as_admin")
    @patch.object(ensure_role_mod, "wait_for_psql_admin_role")
    @patch.object(ensure_role_mod, "bootstrap_app_role_single_user")
    @patch.object(ensure_role_mod, "resolve_psql_admin_role", return_value=None)
    @patch.object(ensure_role_mod, "probe_app_role_exists", return_value=False)
    @patch.object(ensure_role_mod, "probe_postgres_ready", return_value=True)
