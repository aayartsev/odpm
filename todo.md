...
- [ ] архитектура: single-pass resolver зависимостей (OCA `oca_dependencies.txt` дописывает URL в `config.dependencies` во время `map_folders()`, но текущий `for` их в том же проходе не обрабатывает; для полного CI/conf нужен один проход или второй запуск odpm; см. TODO в `build_ci_image()`)